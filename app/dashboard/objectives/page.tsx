...
export default function ObjectivesPage() {
  ...
  async function loadData(userId: string) {
    try {
      ...
      const [objRes, availRes] = await Promise.all([
        apiFetch(`http://localhost:8000/meal/users/${userId}/objectives`),
        apiFetch(`http://localhost:8000/meal/objectifs`),
      ]);
  ...
  async function handleAdd(objectifId: number) {
    ...
    setAdding(true);
    try {
      const res = await apiFetch(`http://localhost:8000/meal/users/${userId}/objectives`, {
        method: "POST",
  ...
  async function handleToggle(obj: Objective) {
    ...
    setToggling(obj.id);
    try {
      const res = await apiFetch(`http://localhost:8000/meal/users/${userId}/objectives/${obj.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
