...
import { Salad, Users, Utensils, Trash2, LogOut, ChevronRight, ChevronDown, Calendar, Ruler, Weight, BadgeInfo, Database, Crown } from "lucide-react";
...
export default function AdminPage() {
  ...
  const updateSubscription = async (userId: number, newSub: string) => {
    console.log(`Tentative de mise à jour abonnement pour user ${userId} -> ${newSub}`);
    try {
      ...
      if (res.ok) {
        ...
        toast.error(`Erreur: ${errData.detail || "Mise à jour échouée"}`);
      }
    } catch (error) {
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      ...
                      <div className="space-y-1">
                        ...
                        <select 
                          ...
                          className={`text-sm font-bold bg-transparent outline-none cursor-pointer border-b border-dashed border-primary/30 hover:border-primary transition-colors ${
                            user.abonnement === 'premium' || user.abonnement === 'premium_plus' ? 'text-amber-600' : 'text-foreground'
                          }`}
                        >
